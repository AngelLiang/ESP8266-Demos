...
int coap_buffer_to_string(char *strbuf, size_t strbuflen, const coap_buffer_t *buf)
{
    ...
    os_memcpy(strbuf, buf->p, buf->len);
    strbuf[buf->len] = 0;
    return 0;
}
...
int coap_build(uint8_t *buf, size_t *buflen, const coap_packet_t *pkt)
{
    ...
    if (pkt->hdr.tkl > 0)
        os_memcpy(p, pkt->tok.p, pkt->hdr.tkl);
    ...
    for (i=0;i<pkt->numopts;i++)
    {
        ...
        *p++ = (0xFF & (delta << 4 | len));
        ...
        os_memcpy(p, pkt->opts[i].buf.p, pkt->opts[i].buf.len);
    ...
    if (pkt->payload.len > 0)
    {
        ...
        os_memcpy(buf+5 + opts_len, pkt->payload.p, pkt->payload.len);
